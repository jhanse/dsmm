...
class BaseAlbum(models.Model):
    ...
    user = models.ForeignKey(User, verbose_name=_('Avtor'), null=True, blank=True, related_name='albums')
    name = models.CharField(_('Ime'), max_length=100, blank=False, null=False)
    created = models.DateTimeField(_('Ustvarjeno'), auto_now_add=True)
    updated = models.DateTimeField(_('Posodobljeno'), auto_now=True)
    is_public = models.BooleanField(_('Javno vidno'), default=True)
